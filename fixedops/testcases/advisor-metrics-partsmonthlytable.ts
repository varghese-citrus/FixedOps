...
function fixedOpsAdvisorMetricsTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0023] ${site.name} FixedOps Advisor Metrics Page Parts Monthly Table Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0023",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await advisorMetricsPagePartsMonthlyTableTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Advisor Metrics Page Parts Monthly Table Test";
...
async function advisorMetricsPagePartsMonthlyTableTest(baseURL: string) {
  let browser = null;
  ...
  try {
    ...
    if (title == "Service Advisor Performance") {
      ...
      const tabs = [as.partsSale, as.partsCost, as.partsProfit, as.markup];
