...
function fixedOpsDiscountSummaryTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0049] ${site.name} FixedOps Discount Summary Labor Page Common Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0049",
          host: Deno.hostname(),
          status: "pass",
        ...
        try {
          ...
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Discount Summary Labor Page Common Test";
