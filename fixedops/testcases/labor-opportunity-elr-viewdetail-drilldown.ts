...
function fixedOpsOpportunityElrPageTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0083] ${site.name} FixedOps Labor Opportunity Effective Labor Rate Page View Detail Drill Down Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0083",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await laborOpportunityElrPageViewDetailDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Labor Opportunity Effective Labor Rate Page View Detail Drill Down Test";
...
async function laborOpportunityElrPageViewDetailDrillDownTest(baseURL: string) {
  let browser = null;
    ...
    if (title == "“What If” Opportunity Effective Labor Rate") {
      ...
      if (graph != null) {
        ...
        if (viewDetainBtn != null) {
          ...
          const title = await page.title();
          const elements = [wl.editBtn, wl.backBtn, wl.dataAsOf];
          ...
          if (title == "Overview") {
            ...
            await page
              .waitForSelector(wl.laborOpportunityContainer, {
                visible: true,
                timeout: 4000,
              })
              .then(() => {
