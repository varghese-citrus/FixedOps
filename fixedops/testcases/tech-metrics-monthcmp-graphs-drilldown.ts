...
function fixedOpsTechMetricsTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0239] ${site.name} FixedOps Tech Metrics Page Month Comparison Graphs Drill Down Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0239",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await techMetricsPageMonthCmpGraphsDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Tech Metrics Page Month Comparison Graphs Drill Down Test";
...
async function techMetricsPageMonthCmpGraphsDrillDownTest(baseURL: string) {
  let browser = null;
  ...
  try {
    ...
    if (title == "Technician Performance") {
      ...
      if (graph) {
        ...
        let id;
        do {
          const idRange1 = await getRandomNumberBetween(3, 12);
          const idRange2 = await getRandomNumberBetween(31, 55);
          const idRange3 = await getRandomNumberBetween(59, 85);
          const idRange4 = await getRandomNumberBetween(87, 113);

          const idRangesArr = [idRange1, idRange2, idRange3, idRange4];
          id = idRangesArr[num - 2];
          try {
            logger.info(`Checking for bar index, iteration ${i}`);
            await page.waitForXPath(as.techMetricsBar(id));
            xpath = await page.$x(as.techMetricsBar(id));
        ...
        if (pageTitle == "Technician Performance") {
          ...
          const dataTable = await page.$eval(as.techPerDataTable, (elem) => {
                          ...
                          await page
                            ...
                            .catch(async () => {
                              ...
                              pageTitle == "Search by Ro"
                                ? logger.info(
                                    "Tech metrics month comparison graph drill down cycle success"
                                  )
                                : [
                                    logger.error(
                                      "Tech metrics month comparison graph drill down cycle failed"
                                    ),
                                    errors.push(
                                      "Tech metrics month comparison graph drill down cycle failed"
                                    ),
