...
function fixedOpsDetailSummaryTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0043] ${site.name} FixedOps Detail Summary Page Labor Revenue Button Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0043",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await detailSummaryPageLaborRevenueBtnTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Detail Summary Page Labor Revenue Button Test";
...
async function detailSummaryPageLaborRevenueBtnTest(baseURL: string) {
  let browser = null;
    ...
    if (role == "admin") {
      ...
      if (title == "Detail Summary") {
        ...
        if (pageHeading == "Detail Summary") {
          ...
          const laborRevenue = await page.waitForXPath(de.cusLaborRevenue, {
            visible: true,
            timeout: 2000,
          });
          await page.waitForTimeout(2000);

          if (laborRevenue != null) {
            logger.info(
              "Customer Labor Revenue By Components visible properly"
            );

            const btns = [de.allBtn, de.withPartsBtn, de.withoutPartsBtn];
            await page.waitForTimeout(2000);
            for (let i = 0; i <= btns.length - 1; i++) {
              const btnXpath = await page.$x(btns[i]);
              const btnName = await (
                await btnXpath[0].getProperty("textContent")
              ).jsonValue();
              await page.waitForTimeout(2000);
              await btnXpath[0].click();
              await page.waitForTimeout(5000);
              ...
              await page
                ...
                .then(() => {
                  logger.info(
                    `data table display properly after clicking ${btnName} button`
                  );
                })
                .catch(() => {
                  logger.error(
                    `data table display not properly after clicking ${btnName} button`
                  );
                  errors.push(
                    `data table display not properly after clicking ${btnName} button`
                  );
            ...
            logger.error(
              "Customer Labor Revenue By Components not visible properly"
            );
            errors.push(
              "Customer Labor Revenue By Components not visible properly"
            );
