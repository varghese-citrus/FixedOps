...
function fixedOpsPartsWorkmixTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0199] ${site.name} FixedOps Parts Workmix Page Opcode Summary Drill Down Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0199",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await partsWorkmixPageOpcodeSummaryDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Parts Workmix Page Opcode Summary Drill Down Test";
...
async function partsWorkmixPageOpcodeSummaryDrillDownTest(baseURL: string) {
  let browser = null;
  ...
  try {
    ...
    if (title == "Parts Work Mix") {
      ...
      await page.click(pw.opcodeSummeryTab);
      await page.waitForTimeout(15000);
      ...
      await page.waitForTimeout(5000);
      await page.waitForSelector(pw.summaryRowData);
                  ...
                  await page
                    ...
                    .catch(async () => {
                      ...
                      pageTitle == "Search by Ro"
                        ? logger.info(
                            "parts workmix opcode summary drill down cycle success"
                          )
                        : [
                            logger.error(
                              "parts workmix opcode summary drill down cycle failed"
                            ),
                            errors.push(
                              "parts workmix opcode summary drill down cycle failed"
                            ),
