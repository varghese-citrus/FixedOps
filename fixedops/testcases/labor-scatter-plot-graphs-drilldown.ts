...
function fixedOpsScatterPlotLaborTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0105] ${site.name} FixedOps Labor Scatter Plot Page Graphs Drill Down Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0105",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await scatterPlotLaborGraphsDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Labor Scatter Plot Page Graphs Drill Down Test";
...
async function scatterPlotLaborGraphsDrillDownTest(baseURL: string) {
  let browser = null;
  try {
    ...
    if (title == "Scatter Plot - Labor Hours / Jobs / ELR") {
      ...
      if (graph) {
        ...
        await page
          ...
          .then(() => {
            logger.info("chart points are added to chart row");
          })
          .catch(() => {
          ...
          if (pageTitle == "Labor Work Mix") {
            ...
            if (attr) {
              ...
              if (pageTitle == "Search by Ro") {
                ...
                await page
                  .waitForSelector(lo.repairOrderTable, {
                    visible: true,
                    timeout: 4000,
                  })
                  .then(() => {
