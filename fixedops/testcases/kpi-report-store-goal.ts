...
function fixedOpsKpiReportStoreGoalTest() {
  getSitesToInspect().forEach((site) => {
    const metrics = site.metrics;
    Deno.test({
      name: `[DEMO-TEST] ${site.name} KPI Report Store Goal Test`,
      fn: async () => {
        ...
        try {
          ...
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps KPI Report Store Goal Test";
  ...
  try {
    ...
    if (actual_title == "ScoreCardGoal") {
      ...
      await page
        ...
        .then(async () => {
          logger.info("store table display properly");
          const storeTableXpath = await page.$x(kpis.storeTableXpath);
          await page.waitForTimeout(2000);
          await page.waitForTimeout(2000);
          ...
          await page
            .waitForXPath(kpis.saveGoalsBtn2, {
              visible: true,
              timeout: 4000,
            })
            .then(() => {
              logger.info("save goal button 2 display properly");
            })
            .catch(() => {
              logger.error("save goal button 2 not display properly");
              errors.push("save goal button 2 not display properly");
            });
        })
        .catch(() => {
          logger.info("store table not properly displayed");
          errors.push("store table not properly displayed");
        });
      const checkBoxStatus = await page.$eval(kpis.emailBoxElement2, (elem) => {
        return elem.style.display == "none";
      });
      if (checkBoxStatus) {
        logger.info("check box unchecked");
        await page.waitForTimeout(4000);
        const checkbox = await page.$x(kpis.checkboxTwo);
        await checkbox[0].click();
        await page.waitForTimeout(4000);
        const checkStatus = await page.$eval(kpis.emailBoxElement2, (elem) => {
          return elem.style.display !== "none";
        });
        checkStatus
        ...
        await page
          .waitForXPath(kpis.addRecipientBtn2, {
            visible: true,
            timeout: 4000,
          })
          .then(async () => {
            logger.info("add recipient button visible properly");
            const btn = await page.$x(kpis.addRecipientBtn2);
            await btn[0].click();
            await page.waitForTimeout(5000);
            await page
...
fixedOpsKpiReportStoreGoalTest();
