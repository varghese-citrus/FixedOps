...
function fixedOpsTechMetricsTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0250] ${site.name} FixedOps Tech Metrics Page Month Trend View Detail Drill Down Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0250",
          host: Deno.hostname(),
          status: "pass",
          pageURL: site.baseURL,
        });
        try {
          await techMetricsPageMonthTrendViewDetailDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Tech Metrics Page Month Trend View Detail Drill Down Test";
...
async function techMetricsPageMonthTrendViewDetailDrillDownTest(
  baseURL: string
) {
  let browser = null;
  ...
  try {
    ...
    if (title == "Technician Performance") {
      ...
      await page
        ...
        .then(() => {
          logger.warn(
            `there is no data in the graph,skipping view detail drill down check`
          );
        })
        .catch(async () => {
          ...
          if (graph) {
            ...
            const viewBtn = await page.$x(
              as.technicianMonthTrendViewDetailBtn(num)
            );
            ...
            if (title == "Overview") {
              ...
              await page
                ...
                .then(() => {
                  logger.info(
                    "tech metrics month trend overview container display properly"
                  );
                })
                .catch(() => {
                  logger.error(
                    "tech metrics month trend overview container not display properly"
                  );
                  errors.push(
                    "tech metrics month trend container not display properly"
                  );
                });
              await page.waitForTimeout(2000);

              await page
                .waitForXPath(as.techMetricsCanvas, {
                  visible: true,
                  timeout: 2000,
                })
                .then(() => {
