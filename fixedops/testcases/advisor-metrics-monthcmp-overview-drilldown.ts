...
function fixedOpsAdvisorMetricsTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0016] ${site.name} FixedOps Advisor Metrics Page Month Comparison Overview Drill Down Test`,
      fn: async () => {
        ...
        try {
          await advisorMetricsPageMonthCmpOverviewDrillDownTest(site.baseURL);
        } catch (error) {
          execMetrics.labels.object.status = "fail";
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Advisor Metrics Page Month Comparison Overview Drill Down Test";
...
async function advisorMetricsPageMonthCmpOverviewDrillDownTest(
  baseURL: string
) {
  let browser = null;
    ...
    if (title == "Service Advisor Performance") {
      ...
      await page
        ...
        .catch(async () => {
          ...
          const graph = await page.waitForXPath(as.getMonthCmpCavas(num1), {
            visible: true,
            timeout: 2000,
          });
          await page.waitForTimeout(2000);
          ...
          if (graph) {
            ...
            await page.waitForTimeout(2000);
            const viewBtn = await page.$x(as.monthCmpViewDetailBtn(num1));
            ...
            if (title == "Overview") {
              ...
              if (canvas) {
                ...
                do {
                  try {
                    num = await getRandomNumberBetween(2, 8);
                    bar = await page.$x(as.bar(num));
                    await bar[0].click();
                    await navigationPromise;
                    ...
                    logger.warn(
                      "there is no data on the graph for drill down,checking for another bar index"
                    );
                    i++;
                  }
                  if (i == 25) {
                ...
                if (
                  pageTitle == "Service Advisor Performance" ||
                  pageTitle != "Overview"
                ) {
                  logger.info(
                                  ...
                                  await page
                                    ...
                                    .catch(async () => {
                                      ...
                                      pageTitle == "Search by Ro"
                                        ? logger.info(
                                            "advisor metrics month comparison overview drill down cycle success"
                                          )
                                        : [
                                            logger.error(
                                              "advisor metrics month comparison overview drill down cycle failed"
                                            ),
                                            errors.push(
                                              "advisor metrics month comparison overview drill down cycle failed"
                                            ),
                  ...
                  logger.error(
                    "Service Advisor Performance title verify failed due to no data in the graph for drill down"
                  );
                  errors.push(
                    "Service Advisor Performance title verify failed due to no data in the graph for drill down"
                  );
