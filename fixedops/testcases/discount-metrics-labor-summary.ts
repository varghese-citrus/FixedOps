...
function fixedOpsDiscountSummaryTest() {
  getSitesToInspect().forEach((site) => {
    ...
    Deno.test({
      name: `[AEC-FOCP-UI-FN-LD-0050] ${site.name} FixedOps Discount Summary  Page Common Test`,
      fn: async () => {
        const execMetrics = metrics.assuranceCaseExec.instance({
          initOn: new Date(),
          txId: metrics.metricsTransactionId,
          caseID: "AEC-FOCP-UI-FN-LD-0050",
          host: Deno.hostname(),
          status: "pass",
        ...
        try {
          ...
          execMetrics.labels.object.statusMessage =
            "Error in FixedOps Discount Summary Page Common Test";
  ...
  try {
    ...
    if (title == "Discount") {
      ...
      heading == "Discount Metrics"
        ? logger.info("Discount Summary Labor page heading verify success")
        : [
            logger.info("Discount Summary Labor page heading verify failed"),
            errors.push("Discount Summary Labor page heading verify failed"),
          ];
      ...
      //to check if the report data table is displaying
      await page
        .$eval(ds.dataTab, (elem) => {
          return elem.style.display !== "none";
        })
        .then(() => {
          logger.info("Discount Summary Labor Data display properly");
        })
        .catch(() => {
          logger.info("Discount Summary Labor Data not properly displayed");
          errors.push("Discount Summary Labor Data not properly displayed");
        });
    } else {
      logger.error("Discount Summary Labor  page title verify failed");
      errors.push("Discount Summary Labor  page title verify failed");
    }

    ts.assert(
      errors.length == 0,
      `Error in  Discount Summary Labor Page: ${errors.join("\n")}`
    );
  } catch (error) {
    console.error(error);
    throw error;
